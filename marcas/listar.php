...
switch ($orden) {
    ...
    case 'nombre_desc':
        ...
        break;
    case 'id_asc':
        $orderBySql = 'id_marca ASC';
        break;
    case 'id_desc':
        $orderBySql = 'id_marca DESC';
        break;
    case 'recientes':
    default:
        $orden      = 'recientes';
...
if ($busqueda !== '') {
    $whereSql = "WHERE nombre LIKE :busqueda";
    $params[':busqueda'] = '%' . $busqueda . '%';
}

// Total registros
$sqlCount = "SELECT COUNT(*) FROM marcas $whereSql";
$stmtCount = $pdo->prepare($sqlCount);
$stmtCount->execute($params);
$total_registros = (int)$stmtCount->fetchColumn();

// Listado
$sql = "SELECT * FROM marcas $whereSql ORDER BY $orderBySql";
...
<div class="page-header-super">
    <div>
        ...
        <small class="text-muted">
            Gestiona las marcas de vehículos. Usa los filtros para encontrar más rápido.
        </small>
    </div>
...
<div class="card">
    <div class="card-body p-0">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-striped align-middle mb-0">
                    ...
                    <?php foreach ($marcas as $m): ?>
                        <tr>
                            <td class="text-muted">#<?= $m['id_marca'] ?></td>
                            <td><strong><?= htmlspecialchars($m['nombre']) ?></strong></td>
                            <td class="text-end">
                                <a href="editar.php?id=<?= $m['id_marca'] ?>"
                                   class="btn btn-sm btn-outline-secondary me-1"
                                   title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a href="eliminar.php?id=<?= $m['id_marca'] ?>"
        ...
        <?php else: ?>
            <div class="p-4">
                <p class="mb-1">No se encontraron marcas.</p>
                <small class="text-muted">Puedes ajustar los filtros o crear una nueva marca.</small>
            </div>
        <?php endif; ?>
    </div>
</div>
